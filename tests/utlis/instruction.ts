...
export async function initialize(
  ...
  const tx = await program.methods
    .initializeRad(initAmount.initAmount0, initAmount.initAmount1, new BN(0))
    .accounts({
...
export async function swap_base_input(
  ...
  const tx = await program.methods
    .swapInput(amount_in, minimum_amount_out)
    .accounts({
      cpSwapProgram: cpSwapProgram,
...
export async function swap_base_output(
  ...
  const tx = await program.methods
    .swapOutput(max_amount_in, amount_out_less_fee)
    .accounts({
      cpSwapProgram: cpSwapProgram,
      payer: owner.publicKey,
      authority: auth,
